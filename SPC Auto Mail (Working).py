...
def fetch_surface_pressure_charts(url=BASE_URL):
    ...
    soup = BeautifulSoup(response.text, 'html.parser')
    get_issue_datetime()  # Print the issued date and time
        ...
        if li_tag:
            ...
            if img:
                ...
                if img_src:
                    ...
                    downloaded_file = download_chart(img_url, i)
                    if downloaded_file:
    ...
    # Send the downloaded files via email
    if downloaded_files:
        send_email_with_attachments(downloaded_files)
...
def send_email_with_attachments(files):
    """Send an email with the downloaded charts as attachments."""
    try:
        msg = EmailMessage()
        msg['Subject'] = "Surface Pressure Charts"
        msg['From'] = EMAIL_ADDRESS
        msg['To'] = RECIPIENT_EMAIL
        msg.set_content("Please find the attached surface pressure charts.")
        ...
        for file in files:
            with open(file, 'rb') as f:
                ...
                msg.add_attachment(file_data, maintype='image', subtype='gif', filename=file_name)

        # Log the email details
        logging.info(f"Preparing to send email to {RECIPIENT_EMAIL} with {len(files)} attachments.")

        # Send the email
        with smtplib.SMTP_SSL('smtp.gmail.com', 465) as smtp:
            smtp.set_debuglevel(1)  # Enable debugging for SMTP
        ...
        logging.error("SMTP Authentication Error: Please check your email address and password.")
